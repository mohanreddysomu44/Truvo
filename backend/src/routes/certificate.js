...
//     });
//   } catch (error) {
//     return res.status(404).json({ error: "Certificate not found", error });
//   }
// });
...
// POST /api/certificate/issue
router.post('/issue', protect, restrictTo('issuer', 'admin'), async (req, res) => {
  try {
    const { learnerName, learnerEmail, learnerWallet, skillName, issuingOrg } = req.body;
    ...
    // STEP 1 — Get next tokenId BEFORE generating PDF
    // so the PDF shows the correct certificate number
    const currentTotal = await getNextTokenId();
    const nextTokenId = currentTotal + 1;

    // STEP 2 — Generate PDF with correct tokenId
    const pdfBuffer = await generateCertificatePDF({
      learnerName,
      skillName,
      issuingOrg: org,
    ...
    // STEP 4 — Mint on blockchain with correct CID
    const { tokenId, txHash } = await mintCertificate({
      learnerWallet, ipfsCid, learnerName, skillName, issuingOrg: org
    });

    // STEP 5 — Save to MongoDB
    await Certificate.create({
      tokenId, ipfsCid, txHash,
      learnerName, learnerEmail, learnerWallet,
      skillName, issuingOrg: org,
      status: 'issued',
    });
    ...
    try {
      await axios.post(process.env.N8N_WEBHOOK_URL, {
        event: 'certificate_issued',
        learnerName, learnerEmail, skillName,
        tokenId, ipfsUrl: getIPFSUrl(ipfsCid), txHash,
      });
    } catch (e) {
      console.log('n8n skipped:', e.message);
    }
...
// POST /api/certificate/revoke/:tokenId
router.post('/revoke/:tokenId', protect, restrictTo('issuer', 'admin'), async (req, res) => {
  try {
    const tokenId = parseInt(req.params.tokenId);
    const { txHash } = await revokeCertificateOnChain(tokenId);
    await Certificate.findOneAndUpdate({ tokenId }, { status: 'revoked' });
