...
export default function IssueCertificate() {
  ...
  if (result) {
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 text-center text-black">
          ...
          <p className="mb-6 text-sm sm:text-base">
            Minted as NFT on the blockchain
          </p>

          <div className="bg-gray-50 rounded-xl p-4 text-left space-y-3 mb-6 text-black">
            <div className="flex justify-between">
              <span className="text-xs sm:text-sm">Token ID</span>
            ...
            <div className="flex justify-between items-center">
              <span className="text-xs sm:text-sm">IPFS CID</span>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 bg-indigo-600 text-white py-2.5 rounded-lg font-medium hover:bg-indigo-700 transition text-center text-sm sm:text-base"
            >
              View Certificate PDF
            </a>
            <button
              onClick={() => setResult(null)}
              className="flex-1 bg-gray-100 text-black py-2.5 rounded-lg font-medium hover:bg-gray-200 transition text-sm sm:text-base"
            >
              Issue Another
            </button>
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 text-black">
      <div className="mb-6">
        <h1 className="text-xl sm:text-2xl font-bold">Issue Certificate</h1>
      ...
      {/* MetaMask connection status */}
      {!isConnected ? (
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between text-black gap-3">
          <div>
            <p className="text-sm font-medium">🦊 Connect MetaMask</p>
          ...
          <button
            onClick={connectWallet}
            className="bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-orange-600 transition w-full sm:w-auto"
          >
            Connect
          </button>
        </div>
      ) : (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-4 text-black">
          <p className="text-sm font-medium">✅ Wallet Connected</p>
          <p className="text-xs sm:text-sm font-mono mt-1 break-all">
            {walletAddress}
          </p>
        </div>
      )}

      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 text-black">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-indigo-500 text-black"
                placeholder="Mohan Kumar"
                required
              />
            </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-xs sm:text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-black"
              placeholder="0x70997970C51812dc3A010C7d01b50e0d17dc79C8"
              required
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-indigo-600 text-white py-3 rounded-lg font-medium hover:bg-indigo-700 transition disabled:opacity-50 flex items-center justify-center gap-2 text-sm sm:text-base"
          >
            {loading ? (
              <>
                <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
